...
export function registerLeadCaptureRoutes(app: Express) {
  ...
  app.get('/api/lead-capture/:websiteId/forms', async (req, res) => {
    try {
  ...
  // Get a specific form
  app.get('/api/lead-capture/:websiteId/forms/:formId', async (req, res) => {
    try {
  ...
  app.post('/api/lead-capture/:websiteId/forms', async (req, res) => {
    try {
      const { websiteId } = req.params;
  ...
  // Update a form
  app.put('/api/lead-capture/:websiteId/forms/:formId', async (req, res) => {
    try {
      const { websiteId, formId } = req.params;
  ...
  // Delete a form
  app.delete('/api/lead-capture/:websiteId/forms/:formId', async (req, res) => {
    try {
      const { websiteId, formId } = req.params;
  ...
  app.get('/api/lead-capture/:websiteId/forms/:formId/html', async (req, res) => {
    try {
      const { websiteId, formId } = req.params;
      const form = await getForm(websiteId, formId);
      ...
      res.status(500).json({
        success: false,
        error: error instanceof Error ? error.message : 'Failed to generate form HTML',
      });
    }
  });
  ...
  // Submit a form
  app.post('/api/lead-capture/:websiteId/forms/:formId/submit', async (req, res) => {
    try {
      const { websiteId, formId } = req.params;
  ...
  app.get('/api/lead-capture/:websiteId/forms/:formId/submissions', async (req, res) => {
    try {
      const { websiteId, formId } = req.params;
  ...
  app.get('/api/lead-capture/:websiteId/submissions/:submissionId', async (req, res) => {
    try {
